"""LoRA fine-tuning for Phase 4 kill-vs-shrink comparison.

Checkpoint directory is keyed on role (low-priority / high-priority), not
GPU count, so a job that restarts at a different world size finds its
...
def train_func(**parameters):
    ...
    # huggingface_hub caches HF_HOME at import time
    output_dir = parameters.get("output_dir", "/mnt/kubeflow-checkpoints")
    os.environ.setdefault("HF_HOME", f"{output_dir}/hf-cache")
    ...
    class GpuSampler:
        ...
        def step_snapshot(self, since_index):
            samples = self._samples[since_index:]
            out = {
    ...
    class ThroughputCallback(TrainerCallback):
        ...
        def on_step_begin(self, args, state, control, **kwargs):
            ...
            self.step_sample_start = len(self.sampler._samples)
        ...
        def on_step_end(self, args, state, control, **kwargs):
            ...
            record = {
                ...
                "warmup": self.steps_since_start <= p.warmup_steps_excluded,
    ...
    run_base = f"phase4-{role}"
    run_name = f"phase4-{role}-{ws}gpu"

    out_dir = Path(p.output_dir) / run_base
    out_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_dir = out_dir / "checkpoints"
    checkpoint_dir.mkdir(parents=True, exist_ok=True)
    metrics_path = out_dir / f"metrics-{ws}gpu-rank{rank}.jsonl"

    if rank == 0:
        print(
            f"[phase4] run={run_name} role={role} world_size={ws} "
    ...
    def _write_timing():
        if rank != 0:
            return
        losses = [r for r in callback.records if r["type"] == "loss"]
        elapsed = time.perf_counter() - train_start if train_start else None
        result = {
            "run_name": run_name,
            "role": role,
            "world_size": ws,
            ...
            "total_train_time_s": round(elapsed, 2) if elapsed and training_finished else None,
            "timeline": list(timeline),
            "losses": losses,
        }
        result.update(sampler.summary_snapshot())
        (out_dir / f"startup_timing_{ws}gpu.json").write_text(json.dumps(result, indent=2))

    callback.timing_writer = _write_timing
    ...
    if rank == 0:
        if resumed:
            print(f"[phase4] resuming from {last_checkpoint}", flush=True)
        else:
            print("[phase4] no checkpoint found, training from scratch", flush=True)
    ...
    if rank == 0:
        print("[phase4] startup timeline:", flush=True)
