"""FSDP fine-tuning of Llama-3.1-8B on Alpaca with per-step throughput recording.

Phase 1 measured the same workload under DDP. This differs in one thing only:
the model is sharded across GPUs with FSDP instead of replicated, so every step
all-gathers the layer weights. Everything else, LoRA, batch geometry, step count
and metrics, matches phase 1 so the two scaling curves are comparable.
"""
...
def train_func(**parameters):
    ...
    p = SimpleNamespace(**{
        ...
        "fsdp_sharding": "full_shard auto_wrap",
    ...
    if rank == 0:
        print(
            f"[phase5] run={run_name} world_size={ws} "
    ...
    sft_config = SFTConfig(
        ...
        fsdp=p.fsdp_sharding,
        fsdp_config={
            # Required when a wrapped block mixes frozen and trainable weights.
            "use_orig_params": True,
            "transformer_layer_cls_to_wrap": [p.fsdp_layer_cls],
    ...
    # Accelerate only selects FSDP for a multi-process launch, so at world size 1
    # the run falls back to plain single-GPU training.
    fsdp_active = bool(getattr(trainer, "is_fsdp_enabled", False))
    if rank == 0:
        print(f"[phase5] fsdp_enabled={fsdp_active} world_size={ws}", flush=True)
    ...
    if rank == 0:
        ...
        summary = {
            ...
            "world_size": ws,
            "parallelism": "fsdp",
            "fsdp_sharding": p.fsdp_sharding,
            "fsdp_enabled": fsdp_active,
            "model_id": p.model_id,
        ...
        print(f"[phase5] summary: {json.dumps(summary, indent=2)}", flush=True)
